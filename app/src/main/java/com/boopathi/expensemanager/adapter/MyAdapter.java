...
/**
 * Created by Boopathi on 3/22/2016.
 */
...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.ViewHolder> {
    ...
    private String profileURL;        //int Resource for header view profile picture
    ...
    public MyAdapter(String Titles[], int Icons[], String Name, String Email, String profileURL, Context passedContext) { // MyAdapter Constructor with titles and icons parameter
        // titles, icons, name, email, profile pic are passed from the main activity as we
        mNavTitles = Titles;                //have seen earlier
        mIcons = Icons;
        name = Name;
        email = Email;
        profileURL = profileURL;                     //here we assign those passed values to the values we declared here
    ...
    public void onBindViewHolder(MyAdapter.ViewHolder holder, int position) {
        if (holder.Holderid == 1) {                              // as the list view is going to be called after the header view so we decrement the
            ...
            Picasso.with(context).load(profileURL).into(holder.profile);// Similarly we set the resources for header view
